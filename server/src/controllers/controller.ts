...
const controller = ({ strapi }: { strapi: Core.Strapi }) => ({
  ...
  async generateBulk(ctx) {
    ...
    if (parallelEnabled) {
      // Parallel processing: collect all images first, then process in batch
      const imageData: Array<{ fileId: number; imageBase64: string }> = [];
      const fileIdToIndex = new Map<number, number>();

      // First pass: validate and collect all images
      ...
      // Process all images in parallel via batch endpoint
      if (imageData.length > 0) {
        try {
          ...
          // Map batch results back to fileIds and update database
            ...
            if (batchResult.success && batchResult.altText) {
              try {
                await strapi.db.query('plugin::upload.file').update({
                  where: { id: fileId },
                  data: { alternativeText: batchResult.altText },
                });
  ...
  async getAutoTagging(ctx) {
    ...
    // Automatically disable if feature is no longer available (e.g., after plan downgrade)
    // This ensures features don't remain enabled when subscription doesn't support them
    if (enabled && !available) {
      await settingsService.setAutoTaggingEnabled(false);
      enabled = false;
    }

    // Note: We don't auto-enable when feature becomes available to respect user choice
    // Users can manually enable features when they upgrade their plan
  ...
  async setAutoTagging(ctx) {
    ...
    // Verify feature is available before enabling
    if (enabled) {
      const subscriptionService = strapi.plugin(PLUGIN_ID).service('subscription');
      const subscription = await subscriptionService.getSubscription();
