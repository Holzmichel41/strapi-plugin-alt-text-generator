...
import { Check, Cross, ExternalLink, GridFour, List, Sparkle, WarningCircle } from '@strapi/icons';
...
const BulkPage = () => {
  ...
  const handleGenerate = async () => {
    ...
    if (parallelProcessingEnabled) {
      // Parallel processing: send all chunks at the same time for maximum speed
      const chunkSize = 5;
      const chunks: number[][] = [];
      ...
      // Create all chunk promises (all sent in parallel)
      ...
      // Process results as they complete (incremental updates)
      // All chunks are already in flight, we await them one by one for incremental UI updates
      for (const promise of chunkPromises) {
        ...
        for (const result of results) {
          ...
          if (result.success) {
            ...
            // Check if the error is usage limit exceeded
      ...
      // Sequential processing: process one by one for incremental status updates
      for (const fileId of ids) {
        try {
          ...
          if (result?.success) {
            ...
            if (result?.error?.includes('Usage limit exceeded')) {
              usageLimitExceeded = true;
              // Mark remaining images as error and break
              setStatuses((prev) => ({ ...prev, [fileId]: 'error' }));
              break;
            }
            setStatuses((prev) => ({ ...prev, [fileId]: 'error' }));
          ...
          // Check if error response contains usage limit message
          const errorMessage =
            error && typeof error === 'object' && 'response' in error
    ...
    // Show upgrade dialog if usage limit was exceeded
    ...
    toggleNotification({
      ...
      message: formatMessage({ id: getTranslation('bulk.success') }, { count: successCount }),
    });
          ...
          <Tr key={image.id} onClick={() => handleSelect(image.id)} style={{ cursor: 'pointer' }}>
            ...
            >
              <Box style={{ pointerEvents: 'none' }}>
                <Checkbox checked={selectedIds.has(image.id)} onCheckedChange={() => { }} />
              </Box>
            </Td>
            <Td>
              <Box
  ...
  return (
    <Layouts.Root>
          ...
          <Dialog.Body>
            <Flex direction="column" alignItems="center" gap={4} padding={4}>
              <Box background="warning100" padding={3} hasRadius style={{ borderRadius: '50%' }}>
                <WarningCircle width={32} height={32} fill="#d9822b" />
              </Box>
      ...
      <Main>
        <Layouts.Header
          title={formatMessage({ id: getTranslation('bulk.title') })}
          subtitle={formatMessage({ id: getTranslation('bulk.description') })}
          primaryAction={
